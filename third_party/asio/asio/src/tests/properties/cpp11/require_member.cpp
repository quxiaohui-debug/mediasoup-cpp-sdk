...
template <int> struct prop { static constexpr bool is_requirable = true; };

template <int> struct object {
  template <int N> constexpr object<N> require(prop<N>) const {
    return object<N>();
  }
};
...
template <int N, int M> struct is_applicable_property<object<N>, prop<M>> {
...
int main() {
  ...
  constexpr object<4> o11 =
      asio::require(object<1>(), prop<2>(), prop<3>(), prop<4>());
  (void)o9;
  (void)o10;
  (void)o11;
}
