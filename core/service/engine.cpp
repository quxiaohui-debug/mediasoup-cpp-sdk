...
std::shared_ptr<RTCContext> Engine::getRTCContext() { return _rtcContext; }
...
std::shared_ptr<IRoomClient> Engine::createRoomClient() {
  ...
  auto RoomClientImpl = std::make_shared<RoomClient>(
      _rtcContext, mediasoupThread, transportThread);
