...
public:
  ParticipantController(rtc::Thread *mediasoupThread,
  ...
  void addObserver(std::shared_ptr<IParticipantEventHandler> observer,
                   rtc::Thread *callbackThread) override;

  void
  removeObserver(std::shared_ptr<IParticipantEventHandler> observer) override;
  ...
  void createParticipant(const std::string &pid,
