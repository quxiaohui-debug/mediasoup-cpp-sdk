...
public:
  ...
  virtual void addObserver(std::shared_ptr<IParticipantEventHandler> observer,
                           rtc::Thread *callbackThread) = 0;

  virtual void
  removeObserver(std::shared_ptr<IParticipantEventHandler> observer) = 0;

  virtual std::shared_ptr<IParticipant>
  getParticipant(const std::string &pid) = 0;

  virtual std::unordered_map<std::string, std::shared_ptr<IParticipant>>
  getParticipants() = 0;
...
PROXY_METHOD2(void, addObserver, std::shared_ptr<IParticipantEventHandler>,
              rtc::Thread *)
PROXY_METHOD1(void, removeObserver, std::shared_ptr<IParticipantEventHandler>)
PROXY_METHOD1(std::shared_ptr<IParticipant>, getParticipant,
              const std::string &)
