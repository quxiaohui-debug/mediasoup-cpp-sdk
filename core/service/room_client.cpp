...
std::string getProtooUrl(const std::string &hostname, uint16_t port,
                         const std::string &roomId, const std::string &peerId) {
  std::stringstream sstr;
  sstr << "wss://" << hostname << ":" << port << "/?roomId=" << roomId
       << "&peerId=" << peerId;
  return sstr.str();
}
...
RoomClient::RoomClient(std::shared_ptr<RTCContext> rtcContext,
                       rtc::Thread *mediasoupThread,
                       rtc::Thread *transportThread)
    : _rtcContext(rtcContext), _mediasoupThread(mediasoupThread),
...
void RoomClient::init() {
  ...
  if (!_mediaController) {
    auto mediaController = std::make_shared<MediaController>(
        _options, _rtcContext->factory(), _mediasoupApi, _mediasoupThread,
        _rtcContext->signalingThread());
  ...
  if (!_participantController) {
    auto participantControllerImpl = std::make_shared<ParticipantController>(
        _mediasoupThread, _mediaController);
    participantControllerImpl->init();
    auto participantControllerProxy = ParticipantControllerProxy::create(
        participantControllerImpl, _mediasoupThread);
    _participantController = std::make_shared<
        ProxyImpl<IParticipantController, ParticipantController>>(
        participantControllerProxy, participantControllerImpl);
...
void RoomClient::addObserver(std::shared_ptr<IRoomClientEventHandler> observer,
                             rtc::Thread *callbackThread) {
  UniversalObservable<IRoomClientEventHandler>::addWeakObserver(observer,
                                                                callbackThread);
}

void RoomClient::removeObserver(
    std::shared_ptr<IRoomClientEventHandler> observer) {
...
void RoomClient::configure() {
  ...
  _peerConnectionOptions->factory =
      peerConnectionFactory != nullptr ? peerConnectionFactory : nullptr;
}

void RoomClient::join(const std::string &hostname, uint16_t port,
...
RoomState RoomClient::getRoomState() { return _state; }

void RoomClient::enableAudio(bool enabled) {
  _mediaController->enableAudio(enabled);
}

void RoomClient::muteAudio(bool muted) { _mediaController->muteAudio(muted); }

bool RoomClient::isAudioMuted() { return _mediaController->isAudioMuted(); }

void RoomClient::enableVideo(bool enabled, const std::string &trackName,
                             uint32_t width, uint32_t height) {
  _mediaController->enableVideo(enabled, trackName, width, height);
}

int32_t RoomClient::speakingVolume() { return _volume; }
...
void RoomClient::getRouterRtpCapabilities() {
  ...
  _mediasoupApi->getRouterRtpCapabilities(
      [wself = weak_from_this()](
          int32_t errorCode, const std::string &errorInfo,
          std::shared_ptr<signaling::GetRouterRtpCapabilitiesResponse>
              response) {
        ...
        if (errorCode != 0) {
          DLOG(
              "getRouterRtpCapabilities failed, error code: {}, error info: {}",
              errorCode, errorInfo);
          return;
        }
...
void RoomClient::requestCreateTransport(bool forceTcp, bool producing,
                                        bool consuming) {
  if (!_mediasoupApi) {
    DLOG("_mediasoupApi is null");
    return;
  ...
  if (_options->datachannel.value_or(false)) {
    ...
    std::string err;
    auto sctpCapabilities = fromJsonString<
        signaling::CreateWebRtcTransportRequest::SCTPCapabilities>(json, err);
    if (!err.empty()) {
  ...
  DLOG("requestCreateTransport, producing: {}, consuming: {}", producing,
       consuming);
  _mediasoupApi->createWebRtcTransport(
      request,
      [wself = weak_from_this(), consuming, producing](
          int32_t errorCode, const std::string &errorInfo,
          std::shared_ptr<signaling::CreateWebRtcTransportResponse> response) {
        auto self = wself.lock();
        if (!self) {
          DLOG("RoomClient is null");
          return;
        ...
        if (errorCode != 0) {
          DLOG("createWebRtcTransport failed, error code: {}, error info: {}",
               errorCode, errorInfo);
          return;
        }
        ...
        DLOG("createWebRtcTransport, producing: {}, consuming: {}", producing,
             consuming);

        self->_mediasoupThread->PostTask(
            RTC_FROM_HERE, [wself, producing, consuming, response]() {
              auto self = wself.lock();
              if (!self) {
                DLOG("RoomClient is null");
                return;
...
void RoomClient::onCreateSendTransport(
...
void RoomClient::onCreateRecvTransport(
    std::shared_ptr<signaling::CreateWebRtcTransportResponse> transportInfo) {
  createTransportImpl(false, true, transportInfo);
  joinImpl();
}

void RoomClient::createTransportImpl(
  ...
  DLOG("createTransportImpl, producing: {}, consuming: {}", producing,
       consuming);
  nlohmann::json iceParameters =
      nlohmann::json::parse(transportInfo->data->iceParameters->toJsonStr());
  ...
  nlohmann::json dtlsParameters =
      nlohmann::json::parse(transportInfo->data->dtlsParameters->toJsonStr());
  nlohmann::json sctpParameters =
      nlohmann::json::parse(transportInfo->data->sctpParameters->toJsonStr());

  if (producing) {
    auto sendTransport = _mediasoupDevice->CreateSendTransport(
...
void RoomClient::joinImpl() {
  ...
  device.flag = "ubuntu";
  device.name = "mediasoup-client-cpp";
  ...
  if (_options->consume.value_or(false)) {
    ...
    std::string err;
    auto rtpCapabilities =
        fromJsonString<signaling::JoinRequest::RTPCapabilities>(json, err);
    if (!err.empty()) {
      DLOG("parse response failed: {}", err);
      return;
    }
  ...
  if (_options->datachannel.value_or(false)) {
    ...
    std::string err;
    auto sctpCapabilities =
        fromJsonString<signaling::JoinRequest::SCTPCapabilities>(json, err);
    if (!err.empty()) {
      DLOG("parse response failed: {}", err);
      return;
    }
  ...
  _mediasoupApi->join(
      request, [wself = weak_from_this()](
                   int32_t errorCode, const std::string &errorInfo,
                   std::shared_ptr<signaling::JoinResponse> response) {
        auto self = wself.lock();
        if (!self) {
          DLOG("RoomClient is null");
          return;
        ...
        if (errorCode != 0) {
          DLOG("join failed, error code: {}, error info: {}", errorCode,
               errorInfo);
          return;
        }
        if (!response || !response->ok) {
        ...
        self->_mediasoupThread->PostTask(
            RTC_FROM_HERE, [wself, state = self->_state, response]() {
              auto self = wself.lock();
              if (!self) {
                DLOG("RoomClient is null");
                return;
              ...
              if (auto pc = self->_participantController->impl()) {
                for (const auto &peer : response->data->peers.value_or(
                         std::vector<signaling::JoinResponse::Peer>())) {
                  pc->createParticipant(peer.id.value_or(""),
                                        peer.displayName.value_or(""));
                }
              }
            });
      });
...
void RoomClient::onRoomStateChanged(vi::RoomState state) {
  ...
  UniversalObservable<IRoomClientEventHandler>::notifyObservers(
      [state](const auto &observer) { observer->onRoomStateChanged(state); });
}
...
void RoomClient::createComponents() {
  ...
  //        std::make_shared<MediaController>(_mediasoupDevice, _mediasoupApi,
  //        _sendTransport, _recvTransport, factory, _options, _mediasoupThread,
  //        signalingThread, adm); mediaController->init();
  //        _signalingClient->addObserver(mediaController);
  //        _mediaController = mediaController;
  //        _mediaController->addObserver(shared_from_this(), _mediasoupThread);
  ...
  //    if (!_participantController) {
  //        auto participantControllerImpl =
  //        std::make_shared<ParticipantController>(_mediasoupThread,
  //        _mediaController); participantControllerImpl->init(); auto
  //        participantControllerProxy =
  //        ParticipantControllerProxy::create(participantControllerImpl,
  //        _mediasoupThread); _participantController =
  //        std::make_shared<ProxyImpl<IParticipantController,
  //        ParticipantController>>(participantControllerProxy,
  //        participantControllerImpl);
  //        _signalingClient->addObserver(participantControllerImpl);
  //        _mediaController->addObserver(participantControllerImpl,
  //        _mediasoupThread);
  //    }
}
...
std::future<void> RoomClient::OnConnect(mediasoupclient::Transport *transport,
                                        const nlohmann::json &dtlsParameters) {
  std::future<void> fut =
      std::async(std::launch::async,
                 [wself = weak_from_this(), transport, dtlsParameters]() {
                   auto self = wself.lock();
                   if (!self) {
                     return;
                   }
...
void RoomClient::OnConnectionStateChange(mediasoupclient::Transport *transport,
                                         const std::string &connectionState) {}

std::future<std::string>
RoomClient::OnProduce(mediasoupclient::SendTransport *transport,
                      const std::string &kind, nlohmann::json rtpParameters,
                      const nlohmann::json &appData) {
  std::future<std::string> fut =
      std::async(std::launch::async, [wself = weak_from_this(), transport, kind,
...
std::future<std::string>
RoomClient::OnProduceData(mediasoupclient::SendTransport *transport,
                          const nlohmann::json &sctpStreamParameters,
                          const std::string &label, const std::string &protocol,
                          const nlohmann::json &appData) {
  std::future<std::string> fut = std::async(
      std::launch::async, [wself = weak_from_this(), transport,
                           sctpStreamParameters, label, protocol, appData]() {
        auto self = wself.lock();
        if (!self) {
          return std::string();
        }
        return self->_onProduceData(transport, sctpStreamParameters, label,
                                    protocol, appData);
      });
...
void RoomClient::_onConnect(mediasoupclient::Transport *transport,
                            const nlohmann::json &dtlsParameters) {
  std::promise<void> promise;
  std::future<void> future(promise.get_future());
  // DLOG("--> [transport], sendTransport: {}, _recvTransport: {}, transport:
  // {}", _sendTransport->GetId(), _recvTransport->GetId(), transport->GetId());
  if (!_mediasoupApi) {
    DLOG("_mediasoupApi is null");
    return;
  }
  ...
  std::string err;
  auto dtlsp =
      fromJsonString<signaling::ConnectWebRtcTransportRequest::DTLSParameters>(
          json, err);
  if (!err.empty()) {
    DLOG("parse response failed: {}", err);
    return;
  ...
  _mediasoupApi->connectWebRtcTransport(
      request, [wself = weak_from_this(), &promise, &transport](
                   int32_t errorCode, const std::string &errorInfo,
        ...
        if (errorCode != 0) {
          DLOG("connectWebRtcTransport failed, error code: {}, error info: {}",
               errorCode, errorInfo);
          promise.set_value();
          return;
        }
...
std::string RoomClient::_onProduce(mediasoupclient::SendTransport *transport,
  ...
  if (appData.contains("sharing")) {
    ...
    request->data->appData->sharing->trackName =
  ...
  std::string err;
  auto rtpp =
      fromJsonString<signaling::ProduceRequest::RTPParameters>(json, err);
  if (!err.empty()) {
    DLOG("parse response failed: {}", err);
    return "";
  }
  request->data->rtpParameters = *rtpp;
  _mediasoupApi->produce(
      request, [wself = weak_from_this(), &promise](
                   int32_t errorCode, const std::string &errorInfo,
                   std::shared_ptr<signaling::ProduceResponse> response) {
        auto self = wself.lock();
        if (!self) {
          DLOG("RoomClient is null");
          promise.set_value("");
        ...
        if (errorCode != 0) {
          DLOG("produce failed, error code: {}, error info: {}", errorCode,
               errorInfo);
          promise.set_value("");
          return;
        }
...
std::string RoomClient::_onProduceData(
  ...
  std::string err;
  auto stcpsp =
      fromJsonString<signaling::ProduceDataRequest::SCTPStreamParameters>(json,
                                                                          err);
  if (!err.empty()) {
    DLOG("parse response failed: {}", err);
    return "";
  ...
  _mediasoupApi->produceData(
      request, [wself = weak_from_this(), &promise](
                   int32_t errorCode, const std::string &errorInfo,
                   std::shared_ptr<signaling::ProduceDataResponse> response) {
        auto self = wself.lock();
        if (!self) {
          DLOG("RoomClient is null");
          promise.set_value("");
        ...
        if (errorCode != 0) {
          DLOG("produceData failed, error code: {}, error info: {}", errorCode,
               errorInfo);
          promise.set_value("");
          return;
        }
...
void RoomClient::onActiveSpeaker(
    std::shared_ptr<signaling::ActiveSpeakerNotification> notification) {
  if (!notification) {
    return;
  }
  ...
  UniversalObservable<IRoomClientEventHandler>::notifyObservers(
      [wself = weak_from_this(), volume = _volume](const auto &observer) {
        auto self = wself.lock();
        if (!self) {
          return;
        }
