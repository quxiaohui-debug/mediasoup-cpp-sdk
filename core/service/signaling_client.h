...
class SignalingClient : public ISignalingClient,
                        ...
                        public UniversalObservable<ISignalingEventHandler>,
...
public:
  ...
  void
  removeObserver(std::shared_ptr<ISignalingEventHandler> observer) override;

  void connect(const std::string &url, const std::string &subprotocol) override;

  void disconnect() override;

  void send(const std::string &text, int64_t transcation, SuccessCallback scb,
            FailureCallback fcb) override;

  void send(const std::vector<uint8_t> &data, int64_t transcation,
            SuccessCallback scb, FailureCallback fcb) override;
