...
ParticipantController::ParticipantController(
    ...
    std::shared_ptr<IMediaController> mediaController)
    : _mediasoupThread(mediasoupThread), _mediaController(mediaController) {}
...
void ParticipantController::destroy() { _participantMap.clear(); }

void ParticipantController::addObserver(
    std::shared_ptr<IParticipantEventHandler> observer,
    rtc::Thread *callbackThread) {
  UniversalObservable<IParticipantEventHandler>::addWeakObserver(
      observer, callbackThread);
}

void ParticipantController::removeObserver(
    std::shared_ptr<IParticipantEventHandler> observer) {
  UniversalObservable<IParticipantEventHandler>::removeObserver(observer);
}

std::shared_ptr<IParticipant>
ParticipantController::getParticipant(const std::string &pid) {
...
std::unordered_map<std::string, std::shared_ptr<IParticipant>>
...
void ParticipantController::createParticipant(const std::string &pid,
  ...
  impl->setActive(true);
  UniversalObservable<IParticipantEventHandler>::notifyObservers(
      [wself = weak_from_this(), participant](const auto &observer) {
...
void ParticipantController::onNewPeer(
    std::shared_ptr<signaling::NewPeerNotification> notification) {
  if (!notification) {
    return;
  }
...
void ParticipantController::onPeerClosed(
    std::shared_ptr<signaling::PeerClosedNotification> notification) {
  if (!notification) {
    return;
  }
...
void ParticipantController::onPeerDisplayNameChanged(
...
void ParticipantController::onActiveSpeaker(
  ...
  /// Math.pow(10, dBs / 20) / However it does not produce a visually useful
  /// output, so let exagerate / it a bit. Also, let convert it from 0..1 to
  /// 0..10 and avoid value 1 to / minimize component renderings.
  ...
  UniversalObservable<IParticipantEventHandler>::notifyObservers(
      [wself = weak_from_this(), participant, volume](const auto &observer) {
        auto self = wself.lock();
        if (!self) {
          return;
        }
...
void ParticipantController::onRemoteAudioStateChanged(const std::string &pid,
                                                      bool muted) {
  if (pid.empty()) {
    return;
  }
  ...
  UniversalObservable<IParticipantEventHandler>::notifyObservers(
      [wself = weak_from_this(), participant, muted](const auto &observer) {
        auto self = wself.lock();
        if (!self) {
          return;
        }
...
void ParticipantController::onRemoteVideoStateChanged(const std::string &pid,
                                                      bool muted) {
  if (pid.empty()) {
    return;
  }
...
void ParticipantController::onCreateRemoteAudioTrack(
...
void ParticipantController::onRemoveRemoteAudioTrack(
    const std::string &pid, const std::string &tid,
    rtc::scoped_refptr<webrtc::MediaStreamTrackInterface> track) {
  if (pid.empty()) {
    return;
...
void ParticipantController::onCreateRemoteVideoTrack(
    const std::string &pid, const std::string &tid,
    rtc::scoped_refptr<webrtc::MediaStreamTrackInterface> track,
  ...
  UniversalObservable<IParticipantEventHandler>::notifyObservers(
      [wself = weak_from_this(), participant, tid, track,
       appData](const auto &observer) {
        auto self = wself.lock();
        if (!self) {
          return;
        }
        observer->onCreateRemoteVideoTrack(participant, tid, track, appData);
      });
}

void ParticipantController::onRemoveRemoteVideoTrack(
    const std::string &pid, const std::string &tid,
    rtc::scoped_refptr<webrtc::MediaStreamTrackInterface> track) {
  if (pid.empty()) {
    return;
  ...
  UniversalObservable<IParticipantEventHandler>::notifyObservers(
      [wself = weak_from_this(), participant, tid,
       track](const auto &observer) {
        auto self = wself.lock();
        if (!self) {
          return;
        }
