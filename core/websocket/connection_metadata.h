...
template <typename T, typename MsgPtr> class ConnectionMetadata;

// 提前声明模板友元函数
template <typename T, typename MsgPtr>
std::ostream &operator<<(std::ostream &out,
                         const ConnectionMetadata<T, MsgPtr> &data);
...
template <typename T, typename MsgPtr>
ConnectionMetadata<T, MsgPtr>::ConnectionMetadata(
    int id, websocketpp::connection_hdl hdl, const std::string &uri,
    std::shared_ptr<IConnectionObserver> observer)
    : _id(id), _hdl(hdl), _status("Connecting"), _uri(uri), _server("N/A"),
      _observer(observer) {}

template <typename T, typename MsgPtr>
void ConnectionMetadata<T, MsgPtr>::onOpen(T *c,
...
template <typename T, typename MsgPtr>
void ConnectionMetadata<T, MsgPtr>::onFail(T *c,
                                           websocketpp::connection_hdl hdl) {
  _status = "Failed";
...
template <typename T, typename MsgPtr>
void ConnectionMetadata<T, MsgPtr>::onClose(T *c,
                                            websocketpp::connection_hdl hdl) {
  _status = "Closed";
  typename T::connection_ptr con = c->get_con_from_hdl(hdl);
  std::stringstream s;
...
template <typename T, typename MsgPtr>
bool ConnectionMetadata<T, MsgPtr>::onValidate(
    T *c, websocketpp::connection_hdl hdl) {
  if (auto observer = _observer.lock()) {
    observer->onValidate();
  }
  return true;
...
template <typename T, typename MsgPtr>
void ConnectionMetadata<T, MsgPtr>::onMessage(T *c, websocketpp::connection_hdl,
                                              MsgPtr msg) {
  if (auto observer = _observer.lock()) {
    if (msg->get_opcode() == websocketpp::frame::opcode::text) {
      observer->onTextMessage(msg->get_payload());
    } else if (msg->get_opcode() == websocketpp::frame::opcode::binary) {
      std::vector<uint8_t> data(msg->get_payload().begin(),
                                msg->get_payload().end());
...
template <typename T, typename MsgPtr>
bool ConnectionMetadata<T, MsgPtr>::onPing(T *c, websocketpp::connection_hdl,
                                           std::string msg) {
  if (auto observer = _observer.lock()) {
    observer->onPing(msg);
  }
...
template <typename T, typename MsgPtr>
void ConnectionMetadata<T, MsgPtr>::onPong(T *c, websocketpp::connection_hdl,
                                           std::string msg) {
  if (auto observer = _observer.lock()) {
    observer->onPong(msg);
  }
...
template <typename T, typename MsgPtr>
void ConnectionMetadata<T, MsgPtr>::onPongTimeout(T *c,
...
template <typename T, typename MsgPtr>
std::ostream &operator<<(std::ostream &out,
                         ConnectionMetadata<T, MsgPtr> const &data) {
  out << "> URI: " << data._uri << "\n"
      << "> Status: " << data._status << "\n"
      << "> Remote Server: "
      << (data._server.empty() ? "None Specified" : data._server) << "\n"
      << "> Error/close reason: "
      << (data._errorReason.empty() ? "N/A" : data._errorReason) << "\n";
  return out;
}
} // namespace vi
