...
public:
  WebsocketRequest(int64_t id, uint32_t timeout)
      : _id(id), _timeout(timeout), _timer(_context) {
    DLOG("request.id = {}", _id);
  }
  ...
  const asio::steady_timer &timer() { return _timer; }
  ...
  void setResolveCallback(ResolveCallback resolve) { _resolve = resolve; }

  void setRejectCallback(RejectCallback reject) { _reject = reject; }
