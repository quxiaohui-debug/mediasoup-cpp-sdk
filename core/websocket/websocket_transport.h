...
template <typename T> WebsocketTransport<T>::~WebsocketTransport() {
  DLOG("~WebsocketTransport()");
}

template <typename T> void WebsocketTransport<T>::init() {}

template <typename T> void WebsocketTransport<T>::destroy() {}

template <typename T> bool WebsocketTransport<T>::isValid() {
...
// ITransport
template <typename T>
void WebsocketTransport<T>::addObserver(
    std::shared_ptr<ITransportObserver> observer) {
  UniversalObservable<ITransportObserver>::addWeakObserver(observer, _thread);
}

template <typename T>
void WebsocketTransport<T>::removeObserver(
    std::shared_ptr<ITransportObserver> observer) {
  UniversalObservable<ITransportObserver>::removeObserver(observer);
}

template <typename T>
void WebsocketTransport<T>::connect(const std::string &url,
  ...
  if (_websocket) {
    _connectionId = _websocket->connect(
        _url, WebsocketTransport<T>::shared_from_this(), subprotocol);
  }
}

template <typename T> void WebsocketTransport<T>::disconnect() {
  if (isValid()) {
...
// IConnectionObserver
template <typename T> void WebsocketTransport<T>::onOpen() {
  DLOG("opened");

  UniversalObservable<ITransportObserver>::notifyObservers(
      [wself = WebsocketTransport<T>::weak_from_this()](const auto &observer) {
...
void WebsocketTransport<T>::onFail(int errorCode, const std::string &reason) {
  ...
  UniversalObservable<ITransportObserver>::notifyObservers(
      [wself = WebsocketTransport<T>::weak_from_this(), errorCode,
       reason](const auto &observer) {
        if (auto self = wself.lock()) {
...
template <typename T> bool WebsocketTransport<T>::onValidate() {
  DLOG("validate");
  return true;
}
...
void WebsocketTransport<T>::onTextMessage(const std::string &json) {
  ...
  UniversalObservable<ITransportObserver>::notifyObservers(
      [wself = WebsocketTransport<T>::weak_from_this(),
       msg = json](const auto &observer) {
        if (auto self = wself.lock()) {
