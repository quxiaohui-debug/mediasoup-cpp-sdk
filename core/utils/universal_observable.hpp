...
namespace vi {
template <typename Observer> class UniversalObservable {
public:
...
protected:
  virtual void
  notifyObservers(std::function<void(const observer_ptr &)> notifier) const {
        ...
        if (obs) {
          ...
          if (thread->IsCurrent()) {
            ...
            thread->PostTask(RTC_FROM_HERE,
                             [wobs = std::weak_ptr<Observer>(obs), notifier]() {
...
private:
  template <typename T = std::shared_ptr<Observer>> class InnerObject {
  public:
