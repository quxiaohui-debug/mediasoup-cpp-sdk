...
#include <vector>
  ...
  protected:
    template <class T>
    void addObserver(std::vector<weak_ptr<T>> &observers,
                     weak_ptr<T> observer) {
    ...
    template <class T>
    void removeObserver(std::vector<weak_ptr<T>> &observers,
                        weak_ptr<T> observer) {
    ...
    template <class T>
    void notify(std::vector<std::weak_ptr<T>> &observers,
                std::function<void(std::shared_ptr<T> &ot)> func) {
      removeInvalidObserver(observers);
      auto copiedObservers = observers;
      for (auto it = copiedObservers.begin(); it != copiedObservers.end();
           ++it) {
