...
template <> struct stringable<std::exception> {
  std::string operator()(const std::exception &e) { return e.what(); }
  std::exception convert_back(const std::string &s) {
    return std::runtime_error(s);
  }
};

template <> struct stringable<std::string> {
...
template <> struct stringable<std::wstring> {
